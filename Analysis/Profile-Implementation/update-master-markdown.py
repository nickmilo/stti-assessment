...
def update_master_markdown():
    ...
    for i, match in enumerate(profile_matches):
        ...
        end_pos = profile_matches[i + 1].start() if i + 1 < len(profile_matches) else len(content)
        profile_section = content[start_pos:end_pos]

        # Extract and update the sticking points section
        stuck_pattern = r'### Sticking Points & Breaking Through.*?\n(.*?)(?=\n### |\n## |\Z)'
        stuck_match = re.search(stuck_pattern, profile_section, re.DOTALL)

        if stuck_match:
            old_section = stuck_match.group(0)
            ...
            if old_section != new_section:
                profile_code = match.group(1) + "-" + match.group(2)
